  ...
  return payload => {
    ...
    if (!questionnaire) {
      ...
      if (score >= LOTTERY_SCORE) {
        scoreMsg += `Поздравляю! Ты можешь получить «ачивку» за задание «Skill-control», для этого пройди на стенд Сбербанка и передай моим коллегам, что испытание пройдено! Они наклеят на твою карточку соответствующий стикер.
...
Спасибо за участие!`;
        ...
        if (MAX_TRY > tryCount) {
          scoreMsg += `Сожалею, но этого не достаточно для получения «ачивки». У тебя есть еще ${MAX_TRY - tryCount} попытки. Чтобы начать заново выполни команду /clear Желаю удачи!`;
        } else {
          scoreMsg += `У тебя больше нет попыток, НО!
Ты все еще можешь получить футболку, если проявишь выдающуюся стойкость в «Игре - просто Бомба!»,
которую мои коллеги проводят в перерывах между докладами на стенде Сбербанка.
Желаю удачи!`
        }
      }
